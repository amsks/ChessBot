...
class Engine_Self_Game():
	...
	def self_play(self):
		if not self.board.is_game_over() and self.board.fullmove_number<=self.maxmoves:
			...
			ret_dict = { 'From': (str(result.move)[0] + str(result.move)[1]), 'To': (str(result.move)[2] + str(result.move)[3]), 'Color': self.color_dict[self.board.turn]}
			return ret_dict
		else:
...
class Human_Engine_Game():
	...
	def Human_Play_uci(self):
		...
		# ret_dict = { 'From': (str(move_uci)[0] + str(move_uci)[1]), 'To': (str(move_uci)[2] + str(move_uci)[3]), 'Color': self.color_dict[self.board.turn] }
		print(self.board)
		# return ret_dict
