...
class Workspace:
    ...
    def movePiece2(self, rob, piece, goal):
        ...
        komo.addObjective(feature=ry.FS.accumulatedCollisions, type=ry.OT.eq, scale=[1e5])
        komo.addObjective([], ry.FS.jointLimits, [], ry.OT.ineq)
        ...
        for i in range(0, 30):
            ...
            #self.C.computeCollisions()  # this calls broadphase collision detection; used for bugfixing
    ...
    def setPieceAside(self, rob, piece):  # ToDo: place in row, not on a pile
        ...
        if rob[0] == 'L':
            #white
            pos = self.C.frame('h1').getPosition()
            x = (pos[0] + int(self.w_taken/2)*0.06)
            y = (pos[1] - 0.1 - (self.w_taken%2)*0.06)
            self.w_taken += 1
            pass
        else:
            #black
            pos = self.C.frame('a8').getPosition()
            x = (pos[0] - int(self.b_taken/2)*0.06)
            y = (pos[1] + 0.1 + (self.b_taken%2)*0.06)
            self.b_taken += 1
            pass

        #toppos = self.C.getFrame(str(rob[0]) + "_gripper").getPosition() + [0., 0., 0.1]
        komo = self.C.komo_path(phases, steps, 2, True)
        komo.addObjective([2.], ry.FS.position, [piece], ry.OT.eq, [3e4], target=[x, y, .9])
        komo.addObjective([1.,2.], ry.FS.position, [piece], ry.OT.sos, [3e5], order=1, target=[0,0,-0.1])
    ...
    def moveHandUp(self, rob, saferelease=False):
        toppos = self.C.getFrame(str(rob[0]) + "_gripper").getPosition() + [0., 0., 0.2]

        komo = self.C.komo_path(1., 15, 2, True)
        #komo.addObjective([1.], ry.FS.position, [str(rob[0]) + "_gripper"], ry.OT.sos, [3e4], target=toppos)
        komo.addObjective([], ry.FS.qItself, ["R_finger1"], ry.OT.eq, [1e4], order=1)
        komo.addObjective([], ry.FS.qItself, ["L_finger1"], ry.OT.eq, [1e4], order=1)
    ...
    def castling(self, rob, king, goal, rook):
        # 1st piece to take goes to [-0.21 -0.25 .7]
        initialPose = self.C.getJointState()
        self.movePiece(rob, king, goal, move_back=False)
        if goal[0] == 'g':
            #-2
            self.movePiece(rob, rook, ('f'+goal[1]), move_back=False)
            pass
        else:
            #+3
            self.movePiece(rob, rook, ('d'+goal[1]), move_back=False)
            pass
        self.moveHandUp(rob)#, saferelease=True)
        self.return2init(initialPose)
    ...
    def initContacts(self):
        ...
        for piece in self.RealWorld.getFrameNames()[len(self.RealWorld.getFrameNames()) - 32:]:
            self.C.frame(piece).setContact(1)
        for piece in self.RealWorld.getFrameNames()[83:len(self.RealWorld.getFrameNames()) - 32]:
            self.C.frame(piece).setContact(0)
        pass
    ...
    def execute_move(self, move, color, movetype=1, other_piece=None):
        ...
        if movetype == 1:
            ...
            print("Castling  commencing...")
            self.castling(robot2move, piece2move, movetile, other_piece)
