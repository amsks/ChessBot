    ...
    for i in range(8):
        for j in range(8):
            if (i+j) % 2:
                ...
                color = black
            position = '(' + str(-3.5*size + j*size) + ' ' + str(3.5*size - i*size) + ' .9)'
            f.write(
                abc[i] + str(j+1) + ' (world){\n' +
                '\tshape:ssBox, Q:<t' + position + '>, size:[.07 .07 ' + str(height) + ' .0001], color:' + color + '\n' +
                '\tmass:2.0\n' +
                '\tcontact, logical:{ }\n' +
                '\tfriction:.1\n' +
                '}\n'
...
def create_pieces():
    ...
    for c in pieces:
        ...
        pose = '[' + str(-3.5 * square_size + i * square_size) + ', ' + str(3.5 * square_size - j * square_size) + ', 1.1, 0, 0, 0, 1]'
        f.write(
