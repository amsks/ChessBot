...
class PGN_Game():
	...
	def getMoveCV(self, i, board):
		...
		pgn = self.moves_assoc[i]['PGN']
		castling = (pgn == 'O-O' or pgn == 'O-O-O')
		piece = self.getPiece(fr, board)
		if castling:
			king = piece
			rook = None
			if to[0] == 'g':
				rook = self.getPiece('h'+to[1], board)
				pass
			elif to[0] == 'c':
				rook = self.getPiece('a'+to[1], board)
				pass

			return (king + "," + to, 3, rook)
...
class Engine_Self_Game():
	...
	def getMoveCV(self, board):
		...
		if not self.board.is_game_over() and self.board.fullmove_number<=self.maxmoves:
			...
			if self.board.has_legal_en_passant() or self.board.has_pseudo_legal_en_passant():
...
class Human_Engine_Game():
	...
	def Human_Play_uci(self):
		...
		if chess.Move.from_uci(move_uci) in self.board.legal_moves:
			...
			self.moves_assoc = { 'From': (str(move_uci)[0] + str(move_uci)[1]), 'To': (str(move_uci)[2] + str(move_uci)[3]), 'Color': self.color_dict[self.board.turn] }
			# print(self.board)
	...
	def getMoveCV(self, board):
		...
		if not self.board.is_game_over() and self.board.fullmove_number<=self.maxmoves:
			...
			if self.turn_var == False:
				move = self.Human_Play_uci()
				self.turn_var = True

			else:
				move = self.Engine_Play()
				self.turn_var = False
